...
class KeyboardVisionAgent:
    ...
    def _loop(self) -> None:
        for step in range(1, self._cfg.max_steps + 1):
            ...
            # Fast-path: generate/edit Excel via a local Python script (avoids cell-by-cell UI typing)
            # to reduce steps and API usage.
            if not self._fastpath_excel_done:
                self._fastpath_excel_done = self._maybe_fastpath_excel(step)
    ...
    def _execute_plan(
        ...
        *,
        ...
        try:
            ...
            if self._cfg.history_log_path:
                self._history.persist_step_jsonl(path=self._cfg.history_log_path, memory=mem)
        except Exception as exc:
    ...
    def _maybe_fastpath_excel(self, step: int) -> bool:
        goal = (self._cfg.goal or "").strip()
        lower = goal.lower()

        if not any(k in lower for k in ("excel", ".xlsx", "workbook", "spreadsheet")):
            return False

        # Only run once per session; keep it conservative.
        filename = self._extract_filename(goal)
        if filename is None:
            if "gtm" in lower and "india" in lower:
                filename = "GTM_India.xlsx"
            else:
                filename = "aik_output.xlsx"
        ...
        # Build some minimal content. If the goal includes explicit content in quotes, use it.
        content_text = self._extract_content(goal)
        if not content_text and "gtm" in lower and "india" in lower:
            content_text = (
                "Go-To-Market Strategy for India\n\n"
                "- Target Market: India\n"
                "- Channels: Digital, Partnerships\n"
                "- Pricing: Intro offers, bundles\n"
                "- Metrics: CAC, activation, retention\n"
            )
        if not content_text:
            content_text = f"Generated by AIK for goal: {goal}"

        script_dir = Path(".aik_runs")
        script_dir.mkdir(parents=True, exist_ok=True)
        script_path = script_dir / f"gen_excel_{self._history.session_id}.py"

        script = self._build_excel_script(target=str(target), text=content_text)
        script_path.write_text(script, encoding="utf-8")

        log.info("Fast-path Excel: generating %s via %s", target, script_path)
        if self._cfg.dry_run:
            # Record in history as a dry-run step.
            mem = self._history.append_step(
                step=step,
                observed="fastpath_excel(dry-run)",
                planned_actions=[{"type": "run_python_script", "path": str(script_path)}],
                executed_actions=[
                    ActionExecutionRecord(
                        step=step,
                        action={"type": "run_python_script", "path": str(script_path)},
                        success=True,
                        duration_ms=0,
                        error=None,
                        timestamp_utc=datetime.now(timezone.utc).isoformat(timespec="seconds"),
            ...
            if self._cfg.history_log_path:
                self._history.persist_step_jsonl(path=self._cfg.history_log_path, memory=mem)
            self._state.human_notes.append(f"FASTPATH: would generate Excel at {target}")
            return True
        ...
        error: str | None = None
        try:
            proc = subprocess.run([sys.executable, str(script_path)], capture_output=True, text=True, timeout=60)
        ...
        rec = ActionExecutionRecord(
            step=step,
            action={"type": "run_python_script", "path": str(script_path), "target": str(target)},
            success=success,
            duration_ms=duration_ms,
            error=error,
            timestamp_utc=datetime.now(timezone.utc).isoformat(timespec="seconds"),
        ...
        mem = self._history.append_step(
            step=step,
            observed=f"fastpath_excel: wrote {target.name}",
            planned_actions=[{"type": "run_python_script", "path": str(script_path)}],
            executed_actions=[rec],
            success=success,
            screenshot_png=b"",
        )
        if self._cfg.history_log_path:
        ...
        if success:
            self._state.human_notes.append(
                f"FASTPATH: Created Excel file at {target}. Do NOT recreate it; proceed to attach/email if needed."
            )
        else:
            log.warning("Fast-path Excel failed: %s", error)
        return success

    @staticmethod
    def _build_excel_script(*, target: str, text: str) -> str:
        ...
        return (
            ...
            "except Exception as e:\n"
            "    raise SystemExit('Missing dependency openpyxl. Run: pip install openpyxl')\n"
            "\n"
